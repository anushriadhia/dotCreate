...
//if username and id is present in path    
 if (count($path_components) >= 5 &&  ($path_components[4] != "")
      ) {

//get userID from path   
    $userid = ($path_components[4]);

    
    $project = "SELECT * FROM Project WHERE UserID ='" . $userid . "'";  
    
      
      $projectResult = mysqli_query($con,$project);
      

     $s = array();
    
    while($row2= mysqli_fetch_array($projectResult))
    {
      $s[]=$row2;
     }
...
//if only username is present in path
    ...
    $currentUser = "SELECT * FROM User WHERE Username ='" . $user . "'";  
